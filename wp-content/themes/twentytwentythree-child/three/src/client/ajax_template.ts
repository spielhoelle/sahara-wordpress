...
const nav_links = document.querySelectorAll("#nav .wp-block-navigation-item__content");
...
const siteBlock = document.querySelector(".wp-site-blocks")
const nav = document.querySelector("#nav")
nav && nav.appendChild(img)
let rndm = 0
    ...
    (link as HTMLAnchorElement).addEventListener('click', (e: MouseEvent) => {
        ...
        if (e.target) {
            ...
            fetch(`http://localhost:8000/wp-json/wp/v2/pages/?slug=${slug}`, {
                ...
                if (data.length > 0) {
                    const siteBlocks = document.querySelector(".wp-site-blocks main");
                    if (siteBlocks) {
                        siteBlocks.innerHTML = data[0].content.rendered;
                        img.classList.add('d-none')
                    }
                }
            })
